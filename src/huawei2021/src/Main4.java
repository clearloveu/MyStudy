...
public class Main4 {
    ...
    private static double[] limitList = {100.0};
//    private static double[] vmlimitList = {0.14, 0.52, 1, 7, 100.0};
    ...
    private static void read2() throws IOException {
        InputStream is = new FileInputStream("C:\\To_be_faster\\IdeaWorkspace\\TestProject1\\src\\huawei2021\\training-2.txt");
    ...
    private static void deal()  {
        ...
        for (int i = 0; i < allRequestArray.length; i++) {
            // 重新选服务器列表
            selectServerList[0] = selectServer(0.0, limitList[0], i);
            for (int j = 1; j < selectServerList.length; j++) {
                selectServerList[j] = selectServer(limitList[j-1], limitList[j], i);
            }
    ...
    private static void deleteVm(Vm currentVm) {
        ...
        int temp = 0;
        while (true){
            if (currentVm.cpuMemoryRate < limitList[temp]) {
    ...
    private static List<String> move(){
        ...
        while (maxMoveSum>0 && index < limitList.length){
            ...
            if (serverListDan.size()>0){
                migrationAnswer = moveDan(serverListDan, maxMoveSum);
                maxMoveSum -= migrationAnswer.size();
                migrationAnswerAll.addAll(migrationAnswer);
            }
            if (serverListShuang.size()>0){
                migrationAnswer = moveShuang(serverListShuang, maxMoveSum);
                maxMoveSum -= migrationAnswer.size();
                migrationAnswerAll.addAll(migrationAnswer);
            }
            index++;
